...
-- Step 2: Drop existing function first (if it exists)
DROP FUNCTION IF EXISTS get_products_with_categories(
    INTEGER, INTEGER, UUID, VARCHAR, TEXT, VARCHAR, VARCHAR, BOOLEAN, BOOLEAN
);

-- Step 3: Create the updated get_products_with_categories function
...
-- Step 4: Ensure all existing products have valid data
UPDATE products 
SET 
...
-- Step 5: Verify the foreign key constraint works
